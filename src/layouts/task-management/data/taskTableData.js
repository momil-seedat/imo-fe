...
export default function Data() {
    ...
    try {
      ...
      if (filterValues.query !== '') {
        response = await axios.get(
          `${apiBaseUrl}/api/search-tasks/?query=${filterValues.query}`, 
          {
            headers: {
  ...
  return {
    ...
    fetchFilteredTasks,
    columns: [
      { Header: "任务名称", accessor: "task_serial", align: "left" },
      { Header: "所属项目名称", accessor: "project", align: "left" },
      { Header: "指定给", accessor: "assigned_to", align: "left" },
     
      { Header: "状态", accessor: "status", align: "center" },
      { Header: "建立日期", accessor: "created_date", align: "center" },
      { Header: "最后更新日期", accessor: "last_updated_at", align: "center" },
    ...
    .map((task, index) => ({
      task_serial: (
        <Link to={userGroup === 'ADMIN' ? `/task-mangement/task_profile/${task.id}` : `/task-mangement/task_profile/${task.id}`}>
         <Author  title={task.name} component="a" name={(userGroup === 'ADMIN' && task.task_serial_no) ?truncateText(task.name,28):truncateText(task.name,28)} /> 
        ...
        project: (
          ...
          {(userGroup === 'ADMIN' && task.project) ? truncateText(task.project.title,28) : task.project_title}
          </MDTypography>
        ),
        assigned_to: (
          <MDTypography component="a" href="#" variant="caption" color="text" fontWeight="medium">
        ...
        edit: (
         <div>{userGroup != "WORKER" && (
          <Link to={userGroup === 'ADMIN' ? `/task-management/update-task/${task.id}` : `/task-management/update-task/${task.task_id}`}>
